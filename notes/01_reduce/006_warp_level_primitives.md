...
**判断标准：Compute Capability ≥ 7.0（Volta 起），就必须用 `_sync` + `mask`。**

| 架构 | 代表 GPU | Compute Capability | 需要 `_sync`？ |
|------|---------|-------------------|---------------|
| Pascal | P100, GTX 1080 Ti | 6.0 / 6.1 | 否（老架构，隐式同步仍可用但不推荐） |
| **Volta** | **V100**, Titan V | **7.0** | **是（独立线程调度的起点）** |
| Turing | RTX 2080 Ti, T4 | 7.5 | 是 |
| Ampere | A100, RTX 3090 | 8.0 / 8.6 | 是 |
| Ada Lovelace | **RTX 4090**, RTX 4080 | 8.9 | 是 |
| Hopper | **H100** | 9.0 | 是 |
| Blackwell | B100, B200 | 10.0 | 是 |

**实务结论：** 目前市面上主流的训练卡（A100、H100）、推理卡（T4、L4）和消费卡（RTX 30/40/50 系列）全部是 Volta 之后的架构。除非你在维护 GTX 1080 Ti 时代的遗留代码，否则**一律按"不信任隐式 Lock-step"来编程**。
