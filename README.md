...
└── 01_parallel_reduction/          # 并行规约优化
    ...
    ├── reduce_v12_warp_level_sums.cu # v12: 寄存器累加 + warp 分层归约
...
| v12 | `reduce_v12_warp_level_sums.cu` | 寄存器累加 + warp 分层归约 | grid-stride 累加到寄存器，warp shuffle → warpLevelSums → 最终 shuffle，shared memory 降至 32B ✅ |
...
**v12 实现要点（寄存器累加 + warp 分层归约）：**
- **核心思想：** v11 仍在 grid-stride loop 中将部分和累加到 shared memory（`sdata[tid]`），然后做 block 级树形归约（3~4 次 `__syncthreads()`）。v12 将累加目标从 shared memory 改为**寄存器**（`float sum`），归约阶段采用**两级 warp shuffle**，shared memory 仅用于 warp 间中转
- **四步归约流程：**
  ```
  ① grid-stride loop → 寄存器累加（零 shared memory 写入）
  ② warp 内 shuffle 归约（32 个线程 → 1 个部分和）
  ③ 每个 warp 的 lane 0 写入 warpLevelSums[]（仅 8 个 float = 32 B）
  ④ __syncthreads() → 第一个 warp 读 warpLevelSums[] → shuffle 最终归约
  ```
- **对比 v11 的改进：**

  | | v11 | v12 |
  |---|---|---|
  | grid-stride 累加目标 | shared memory (`sdata[tid]`) | **寄存器** (`float sum`) |
  | block 级归约方式 | shared memory 树形归约 (256→128→64) | **warp shuffle → warpLevelSums → shuffle** |
  | shared memory 用量 | 256 floats (1 KB) | **8 floats (32 B)** |
  | `__syncthreads()` 次数 | 3~4 次 | **1 次** |

- **工程价值：** 在 reduce 这个纯带宽瓶颈场景中，v12 与 v11 吞吐持平。但 v12 的"寄存器累加 + warp 分层归约"模式在**复杂算子**中极为实用——当 block 内需要对中间结果做局部 reduce 时（如 LayerNorm、Softmax），极低的 shared memory 占用和极少的同步点能显著提升 occupancy 和并发度
...
|------|-----------|-------------|------|-------------|
| v0 | 2.849 | 378.33 | 37.53% | baseline |
| v1 | 2.462 | 437.85 | 43.44% | +15.7% |
| v2 | 2.515 | 428.68 | 42.53% | +13.3% |
| v3 | 1.707 | 631.61 | 62.66% | **+66.9%** |
| v4 | 1.191 | 903.34 | 89.62% | **+138.7%** |
| v5 | 1.196 | 901.58 | 89.44% | **+138.2%** |
| v6 | 1.190 | 903.88 | 89.67% | **+138.8%** |
| v7 | 1.191 | 903.53 | 89.64% | **+138.8%** |
| v7b | 1.191 | 903.57 | 89.64% | **+138.8%** |
| v8 | 1.187 | 904.46 | 89.73% | **+139.0%** |
| v9 | 1.200 | 895.08 | 88.80% | **+136.6%** |
| v10 | 1.200 | 894.97 | 88.79% | **+136.5%** |
| v11 | 1.187 | 904.65 | 89.75% | **+139.1%** |
| v12 | 1.188 | 904.19 | 89.70% | **+139.0%** |

v4-v12 在 Cold Cache + 大数据量条件下性能非常接近（~895-905 GB/s，89-90%），说明在数据量远超 L2 缓存时，**内存带宽已成为绝对瓶颈**，计算侧的优化（warp 展开、模板展开、grid-stride loop、shuffle、warp 分层归约）收益趋于饱和。v12 将整个归约路径改为寄存器累加 + 两级 shuffle，shared memory 占用从 1 KB 降至 32 B，`__syncthreads()` 从 3~4 次降至 1 次，核心价值在于为复杂算子（LayerNorm、Softmax 等）内嵌归约提供更友好的资源占用。v9/v10 用 Occupancy API 自动决定 gridSize/blockSize，虽然在 RTX 4090 上性能与手动调参的 v8 接近（差 ~1%），但核心价值在于**跨架构可移植性**——换一块 GPU 无需修改代码即可获得接近最优的配置。
