...
|------|-----------|-------------|------|-------------|
| v0 | 2.866 | 376.09 | 37.31% | baseline |
| v1 | 2.462 | 437.86 | 43.44% | +16.4% |
| v2 | 2.515 | 428.60 | 42.52% | +14.0% |
| v3 | 1.708 | 631.27 | 62.63% | **+67.8%** |
| v4 | 1.193 | 902.06 | 89.49% | **+139.9%** |
| v5 | 1.197 | 900.56 | 89.34% | **+139.5%** |
| v6 | 1.192 | 902.40 | 89.52% | **+139.9%** |
| v7 | 1.193 | 901.94 | 89.48% | **+139.8%** |
| v7b | 1.191 | 902.99 | 89.58% | **+140.1%** |
| v8 | 1.188 | 903.69 | 89.65% | **+140.3%** |
| v9 | 1.204 | 891.93 | 88.48% | **+137.2%** |
| v10 | 1.205 | 891.21 | 88.41% | **+136.9%** |
| v11 | 1.188 | 904.08 | 89.69% | **+140.4%** |

v4-v11 在 Cold Cache + 大数据量条件下性能非常接近（~891-904 GB/s，88-90%），说明在数据量远超 L2 缓存时，**内存带宽已成为绝对瓶颈**，计算侧的优化（warp 展开、模板展开、grid-stride loop、shuffle）收益趋于饱和。v11 用 Shuffle 替代 volatile Shared Memory 做 warp 归约，带宽与 v8 持平（904 vs 904 GB/s），主要收益在于**代码安全性**（消除 volatile 隐患）和**架构兼容性**（Volta+ 不再依赖 warp 隐式同步）。v9/v10 用 Occupancy API 自动决定 gridSize/blockSize，虽然在 RTX 4090 上性能与手动调参的 v8 接近（差 ~1%），但核心价值在于**跨架构可移植性**——换一块 GPU 无需修改代码即可获得接近最优的配置。
